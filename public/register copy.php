...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO users (email, password, role, name, phone_number, profile_picture) VALUES (?, ?, 'tutor', ?, ?, ?)";
    ...
    $stmt->bind_param("sssss", $email, $password, $name_en, $mobile_number, $photo);
    ...
    $sql = "INSERT INTO tutors (user_id, name_en, name_bn, father_name_en, mother_name_en, mobile_number, dob, birthplace, nationality, nid_number, nid_copy, passport_number, passport_expiry_date, passport_copy, gender, marital_status, mailing_address, permanent_village, permanent_post_office, permanent_police_station, permanent_upazilla, permanent_district, present_village, present_post_office, present_police_station, present_upazilla, present_district, photo, signature, cv) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    // Log the SQL query
    error_log("SQL Query (tutors): " . $sql);

    $stmt = $conn->prepare($sql);
    if ($stmt === false) {
        error_log("Prepare failed (tutors): " . $conn->error);
        die("Prepare failed (tutors): " . $conn->error . " - " . $conn->error);
    }

    // Log the bound parameters
    error_log("Bound Parameters (tutors): " . json_encode([
        $user_id, $name_en, $name_bn, $father_name_en, $mother_name_en, $mobile_number, $dob, $birthplace, $nationality, $nid_number, $nid_copy, $passport_number, $passport_expiry_date, $passport_copy, $gender, $marital_status, $mailing_address, $permanent_village, $permanent_post_office, $permanent_police_station, $permanent_upazilla, $permanent_district, $present_village, $present_post_office, $present_police_station, $present_upazilla, $present_district, $photo, $signature, $cv
    ]));

    $stmt->bind_param("isssssssssssssssssssssssssssss", $user_id, $name_en, $name_bn, $father_name_en, $mother_name_en, $mobile_number, $dob, $birthplace, $nationality, $nid_number, $nid_copy, $passport_number, $passport_expiry_date, $passport_copy, $gender, $marital_status, $mailing_address, $permanent_village, $permanent_post_office, $permanent_police_station, $permanent_upazilla, $permanent_district, $present_village, $present_post_office, $present_police_station, $present_upazilla, $present_district, $photo, $signature, $cv);

    if ($stmt->execute() === false) {
        error_log("Execute failed (tutors): " . $stmt->error);
        die("Execute failed (tutors): " . $stmt->error);
    }
